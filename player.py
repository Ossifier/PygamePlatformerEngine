...
class Player(pygame.sprite.Sprite):
    ...
    def get_player_inputs(self):
        ...
        acc_spd_correct = self.move_acceleration * self.game_speed

        if keys[pygame.K_RIGHT] and self.direction.x >= self.game_speed:      # Running Right
            if self.direction.x >= self.max_move_speed:
                pass
            else:
                self.direction.x += acc_spd_correct
        elif keys[pygame.K_RIGHT] and self.direction.x <= self.game_speed:
            self.direction.x += acc_spd_correct * 2
        elif keys[pygame.K_LEFT] and self.direction.x <= self.game_speed:     # Running Left
            if self.direction.x <= -self.max_move_speed:
                pass
            else:
                self.direction.x -= acc_spd_correct
        elif keys[pygame.K_LEFT] and self.direction.x >= self.game_speed:
            self.direction.x -= acc_spd_correct * 2
        ...
        if keys[pygame.K_SPACE]:
            if self.on_ground is True and self.current_jump_power >= 6:
                ...
                self.current_jump_power -= self.game_speed
            else:
                self.jumping = False
    ...
    def assign_player_states(self):
        ...
        if self.direction.y > 1:
            ...
            if self.on_ground is False and self.jumping is False:
                ...
                animate.animate_sprite_slice(sprite, 6, 9)              # Move this inside animate fcn?
    ...
    def apply_gravity(self):
        ...
        self.rect.y += self.direction.y * self.game_speed
    ...
    def jump(self):
        ...
        if self.on_ground is True and self.stamina > 0 and self.current_jump_power > 0:
            self.jumping = True
            self.on_ground = False
            self.direction.y = self.jump_speed + self.game_speed         # THIS WORKS???
    ...
    def jump_power_handler(self):
        ...
        if self.current_jump_power > self.max_jump_power:                               # Value Overflow Handling.
            self.current_jump_power = self.max_jump_power
        elif self.current_jump_power < 0:
            self.current_jump_power = 0

        if self.current_jump_power < self.max_jump_power and self.on_ground is True:    # Recharge Stamina on Ground.
            self.current_jump_power += self.jump_recharge_rate * self.game_speed
